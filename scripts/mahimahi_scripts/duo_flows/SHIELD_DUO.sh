...
# ports: derived from base_port + 2*idx and +1
...
next_index() {
  ...
  for f in "$OUT_DIR"/output_duo_*.txt; do
    local base="${f##*/}"          # output_duo_<idx>.txt
    local num="${base##*_}"        # <idx>.txt
    num="${num%.txt}"
    [[ "$num" =~ ^[0-9]+$ ]] || continue
    (( num > max )) && max="$num"
  done
...
run_mahi_duo_root() {
  ...
  # We are ALWAYS running an L4S TCP flow in this duo setup
  ensure_l4s_prague
  ...
  # Ensure files writable by non-root Mahimahi user
