...
# Purpose
# ============================================================
# - Parse Mahimahi + Linux qdisc logs into comparable time-series
# - Compute DTW distances (normalized by warping path length)
# - Cache DTW results (within-qdisc, within-mahimahi, cross)
# - Plot histograms/CDFs/overlays
# - NEW: Bootstrap CI for p_max using *run-ID resampling* (correct level)
#
# Key bootstrap idea:
#   Resample run IDs (e.g. "12l", "34m") with replacement, rebuild:
#     - within distance samples from cached pairwise dicts
#     - cross distance sample from cached cross dict
#   Then recompute eps_max and p_max for each bootstrap replicate.
# ============================================================
...
from validation_steps.min import _render_validation_histogram
...
class DTWAnalyzer:
    ...
    def __init__(self, qdisc_dir, mahi_dir, mode="packets"):
        ...
        # Optional trace caches (for overlay plotting)
    ...
    # ============================================================
    # Trace reading (for overlay plots)
    # ============================================================
    ...
    def _qdisc_keys(self):
        # Prefer dict keys; fall back to list if needed
    ...
    # ============================================================
    def _within_dists_for_sample(self, keys_sample, dist_dict):
        """
    ...
    def _cross_dists_for_sample(self, q_sample, m_sample):
        """
    ...
    def bootstrap_ci_pmax(self, *, quantile=95.0, B=2000, alpha=0.05, seed=0):
        """
        Bootstrap CI for p_max using *run-ID resampling*.

        This is the correct bootstrap level for your setup:
          - resample runs (IDs), not pairwise distances
          - rebuild within/cross distance samples from cached dicts
        ...
        q_keys = self._qdisc_keys()
        m_keys = self._mahi_keys()
        ...
        for b in range(B):
            ...
            q_within = self._within_dists_for_sample(q_s, self.qdisc_dist)
            m_within = self._within_dists_for_sample(m_s, self.mahi_dist)
            cross    = self._cross_dists_for_sample(q_s, m_s)
    ...
    # ============================================================
    # Plotting: triple hist, validation hist, CDFs, overlays
    # ============================================================
    def plot_triple_hist(self):
        ...
        # Validation-only (unchanged)
        fig_val = plt.figure(figsize=(7.5, 4))
        ax = plt.gca()
        ...
        if np.isfinite(eps_min):
            ax.axvline(eps_min, linewidth=1.5, linestyle="--")
        if np.isfinite(eps_max):
            ax.axvline(eps_max, linewidth=1.5, linestyle="--")

        ax.set_xlabel("Cross-mode DTW")
        ax.set_ylabel("count")
        ...
        ax.text(0.5, 1.03, header, transform=ax.transAxes, ha="center", va="bottom")
    ...
    def bootstrap_ci_pmax_for_keys(
        ...
        *,
        ...
        """
        Same as bootstrap_ci_pmax(), but restricted to the provided key lists.
    ...
    def print_bootstrap_ci_vs_n(
        ...
        *,
        ...
        q_all = list(self._qdisc_keys())
        m_all = list(self._mahi_keys())

        if not q_all or not m_all:
            print("No keys available.")
            return []

        # Shuffle once so the subset selection is deterministic from `seed`
        rng.shuffle(q_all)
        rng.shuffle(m_all)
        ...
        for n in ns:
            ...
            p_lo, p_hi, p_upper_1s = self.bootstrap_ci_pmax_for_keys(
                q_sub,
                m_sub,
                quantile=quantile,
                B=B,
                alpha=alpha,
                seed=seed,  # keep seed fixed so only n changes
            )
    ...
    def plot_bootstrap_ci_vs_n(
        ...
        *,
        ...
        # Collect CI widths
        # --------------------------------------------
        results = self.print_bootstrap_ci_vs_n(
            ns=ns,
            quantile=quantile,
            B=B,
            alpha=alpha,
        ...
        ax.set_title(
            rf"Bootstrap {(1-alpha)*100:.0f}% CI Width vs Sample Size",
            fontsize=TITLE_SIZE,
            pad=15,
        )
    ...
    def plot_validation_hist(self, include_x_axis_label=False, include_y_axis_label=False):
        qvals = np.asarray(self._extract_values(self.qdisc_internal), float)
        mvals = np.asarray(self._extract_values(self.mahi_internal), float)
        cvals = np.asarray(self._extract_values(self.cross_results), float)
        ...
        # bootstrap CI for p_max
        # p_lo, p_hi, p_upper_1s = self.bootstrap_ci_pmax(quantile=95.0, B=2000, alpha=0.05, seed=0)

        # self.print_bootstrap_ci_vs_n(ns=(10, 25, 40, 60, 80, 100), B=2000, alpha=0.05, seed=0)
        # base_dir = Path("figs")
        # base_dir.mkdir(parents=True, exist_ok=True)

        # base_name = f"{self.variable}_ci_width_vs_n"
        # out_path = base_dir / f"{base_name}.svg"

        # # If file exists, append _2, _3, ...
        # counter = 2
        # while out_path.exists():
        #     out_path = base_dir / f"{base_name}_{counter}.svg"
        #     counter += 1

        # self.plot_bootstrap_ci_vs_n(out_path=out_path)
        # print("Bootstrap CI for p_max: [{:.4f}, {:.4f}], upper 1-sided {:.4f}".format(p_lo, p_hi, p_upper_1s))
        # print("original p_max: {:.4f}".format(p_max), "original p_min: {:.4f}".format(p_min))
        ...
        return _render_validation_histogram(
            ...
            p_min,
            p_max,
            # CHANGED: pass CI into renderer
            # p_ci_lo=float(p_lo),
            # p_ci_hi=float(p_hi),
            ci_alpha=0.05,
            # p_upper_1s=float(p_upper_1s),
            x_label="Normalized DTW Distance",
