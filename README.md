...
These create:
    output_0.txt, output_1.txt, ...   (Mahimahi)
    qdisc_0.log, qdisc_1.log, ...     (Kernel)
...
7. Run Parametric Test
    Permutation test to measure whether Mahimahi and Kernel mean DTW
...
BYTES MODE:
    ε ≈ average allowed bytes difference per sample

PACKETS MODE:
    ε ≈ average allowed packets difference per sample
...
A reproducible, statistically validated pipeline to assess whether
Mahimahi’s DualPI2 queue behavior matches Linux kernel DualPI2. This
includes:
